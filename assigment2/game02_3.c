...
int main(int argc, char *argv[]){
        ...
        if(id[i] < 0){
         ...
         //child process
            ...
            while(1){
                ...
                long int try_lock_time = child_current_time.tv_usec + child_current_time.tv_sec * 1000000; 
                //printf("%d start to try lock at %ld\n",getpid(),try_lock_time);
                sem_wait(&(data->sem));
                gettimeofday(&child_current_time,NULL);
                long int get_lock_time = child_current_time.tv_usec + child_current_time.tv_sec * 1000000;
            ...
            if(i==0){
                // start time of parent's process
                gettimeofday(&parent_current_time,NULL);
                start_time = parent_current_time.tv_usec + parent_current_time.tv_sec * 1000000;
            }
        }                 
    }
    ...
    // end time of parent's process
    gettimeofday(&parent_current_time,NULL);
    end_time = parent_current_time.tv_usec + parent_current_time.tv_sec * 1000000; 
    // print the result 
    printf("winner value:%d\n",max);
    printf("winner pid:%d\n",data->winner);
    //printf("%d starts at %ld\n",getpid(),start_time);
    //printf("%d ends at %ld\n",getpid(),end_time);
